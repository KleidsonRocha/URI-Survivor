...
public class EnemyController : MonoBehaviour
{
    ...
    public float healthPickupDropRate = 0.1f;
    ...
    void Update()
    {
        if (PlayerController.Instance.gameObject.activeSelf == true)
        {
            if (knockBackCounter > 0)
            {
                if (isBoss)
                {
                    knockBackCounter = 0; // Se for um chefe, cancela o knockback. Chefes sao imunes.
                }
                else
                {
                    knockBackCounter -= Time.deltaTime;

                    // Logica de movimento reverso durante o knockback
                    if (moveSpeed > 0)
                    {
    ...
    public void TakeDamage(float damageToTake)
    {
        ...
        if (health <= 0)
        {
            ...
            if (pickupAllCoinPrefab != null && Random.value <= pickupAllCoinDropRate)
            {
            ...
            if (healthPickupPrefab != null && Random.value <= healthPickupDropRate)
            {
                Instantiate(healthPickupPrefab, transform.position + new Vector3(.2f, .1f, 0f), Quaternion.identity);
                Debug.Log("Inimigo dropou um pickup de cura!");
            }
